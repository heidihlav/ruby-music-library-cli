class MusicLibraryController
    ...
    def list_songs_by_artist
      ...
      if artist_name = Artist.find_by_name(input)
        artist_name.songs.sort{ |a, b| a.name <=> b.name }.each.with_index(1) do |s, i|
    ...
    def list_songs_by_genre
      ...
      if genre_name = Genre.find_by_name(input)
        genre_name.songs.sort{ |a, b| a.name <=> b.name }.each.with_index(1) do |s, i|
    ...
    def play_song
      ...
      if (1..Song.all.length).include?(input)
        if song = Song.all.sort { |a, b| a.name <=> b.name }[input - 1]
